...
function edit(event){
	...
	let thought_id = document.getElementById("input_edit_id").value;
	let thought_text = document.getElementById("textarea-edit_thought").value;

	//TODO 

	fetch(base_url+"edit?id="+encodeURIComponent(thought_id)
	+"&token="+encodeURIComponent(me.token)
	+"&thought="+encodeURIComponent(thought_text)
	)
	.then(function(response) {
		return response.json();
	})
...
function editThought(event){
	let originator = event.currentTarget;
	let thought_id = originator.getAttribute('data-thought-id');
	let thought_text = originator.getAttribute('data-thought-text');
	console.log("edit id="+thought_id);

	editModal.show();
	
	document.getElementById("input_edit_id").value = thought_id;
	document.getElementById("textarea-edit_thought").value = thought_text;
}
