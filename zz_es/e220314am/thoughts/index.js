	...
	try{
		...
		for(li=0; li<sobj.length; li++){
			if(sobj[li].id == req.query.id){
				...
				if(sobj[li].nickname != user.nickname) throw('security issue edit aborted');
		...
		if(!found) throw('thoughts to edit not found');
	...
	try{
		...
		for(li=0; li<sobj.length; li++){
			if(sobj[li].id == req.query.id){
				...
				if(sobj[li].nickname != user.nickname) throw('security issue delete aborted');
